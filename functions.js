...
const qtdEmDestaque = (produtos) => {
    let soma = 0
    for (var i = 0; i < produtos.length; i++){
        if (produtos[i].emDestaque === "sim") {
            soma = soma + produtos[i].qtdEstoque
        }
    }
    console.log(`2. Quantidade total de itens em destaque ${soma}`)
    return soma
}
qtdEmDestaque(listaProdutos)
...
const produtoMaiscaro = (produtos) => {
    ...
    let prodMaiorPreco
    let prodMaiorPrecoDepto
    for (var i = 1; i < produtos.length; i++){
        if (produtos[i].preco === maiorPreco){
            prodMaiorPreco = produtos[i].descricao
            prodMaiorPrecoDepto = produtos[i].departamento.nomeDepto
        }
    }
    console.log(`6. Produto mais caro da loja é ${prodMaiorPreco} do departamento ${prodMaiorPrecoDepto} que tem o preço ${maiorPreco}`)
    return maiorPreco
}
produtoMaiscaro(listaProdutos)

// 7. Produto mais barato da loja (bem como seu departamento - considere apenas o preço dele)
function ex7(maisBarato) {
    maisBarato = listaProdutos.map(produto => produto.preco).reduce((a, b) => Math.min(a, b))
    nomeMaisBarato = listaProdutos.map(produto => {
        if (maisBarato == produto.preco) {
            nomeProdutoMaisBarato = produto.descricao
            nomeDepartamento = produto.departamento.nomeDepto
        }
    })
    console.log(`7. O produto ${nomeProdutoMaisBarato} é o item mais barato do estoque. Ele custa R$ ${maisBarato} e pertence ao departamento de ${nomeDepartamento}`)
}
ex7()
